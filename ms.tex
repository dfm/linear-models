...
For our purposes, a \emph{linear model} for data $y$ (or differences $y$ between data and
a mean model or expectation for those data) can be written in the form
\begin{align}
\eqlabel{linearmodel}
\bvec{y} = \mA \vw + \mathrm{noise}
\end{align}
where $\mA$ is a \emph{design matrix}, whose columns are basis
vectors $\{\bvec{a}_j\}$, and $\vw$ is the vector of weights or amplitudes, one
for each basis vector.

In this \documentname, we show that if a
Gaussian (or else extremely wide) prior is placed on the weights $\vw$ of the linear
