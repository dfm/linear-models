...
\citep{Tsalmantza:2012, Ness:2015}, and imaging (Cite new Wang, Cite Fergus S5)
\alltodo{ADD MORE REFERENCES}.
...
\eqlabel{integral}
p(\vy | \vt) 
%             &= \int_{-\infty}^{\infty} p(\vw) p(\vy | \vt, \vw) \dd\vw \nonumber \\
%
%             &= \int_{-\infty}^{\infty} \Gaussian{\vw}{\mL}{K} \nonumber\\
%             & \quad\quad\quad \times \Gaussian{(\vre - \mA \vw)}{\mC}{N}
%               \dd\vw \nonumber \\
%
             &= \frac{1}{|2\pi\mL|^\frac{1}{2} |2\pi\mC|^\frac{1}{2}}
...
By the Matrix Determinant Lemma and the Woodbury Identity \citeme,
%
\begin{align}
